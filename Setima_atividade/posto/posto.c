...
void processaLoteAmostras(Posto* posto){
    while(1){
        Amostra* a = retiraItemLista(posto->amostras);
        if(a == NULL) break;
        if(retornaCargaViral(a) >= LIMITE_CARGA_VIRAL){
            registraResultado(a, POSITIVO);
            if(retornaIdade(a) >= 60) insereInicioFila(posto->positivas, a);
            else insereFimFila(posto->positivas, a);
        }
        else{
            registraResultado(a, NEGATIVO);
            if(retornaIdade(a) >= 60) insereInicioFila(posto->negativas, a);
            else insereFimFila(posto->negativas, a);
        }
    }
}
